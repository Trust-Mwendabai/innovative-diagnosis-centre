    ...
    return (
        <div className="space-y-10 animate-fade-in pb-12">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    <h1 className="text-5xl font-black tracking-tighter text-white italic">
                        Expert <span className="text-cyan-500 underline decoration-[hsl(var(--gold))] underline-offset-8">Intelligence</span>
                    </h1>
                    <p className="text-white/60 font-bold mt-3 text-base italic">Welcome back, Dr. {user?.name}. Here is your clinical summary.</p>
                </div>
                <div className="flex items-center gap-3">
                    <div className="text-right hidden sm:block">
                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40">System Status</p>
                    ...
                    <div className="flex items-center gap-6 relative z-10">
                        ...
                        <div>
                            <h4 className="font-black text-white text-base uppercase tracking-widest mb-1">
                                {notifications.length} New Clinical Announcement{notifications.length > 1 ? 's' : ''}
                            </h4>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {stats.map((stat, i) => (
                    <Card key={i} className="glass-card border-white/10 shadow-2xl rounded-[2.5rem] overflow-hidden group hover:scale-[1.05] transition-all duration-500">
                        <CardContent className="p-8">
                            <div className="flex justify-between items-start mb-6">
                                <div className={cn("p-4 rounded-2xl bg-white/5 group-hover:bg-cyan-500/20 transition-colors duration-500", stat.color)}>
                                    <stat.icon className="h-7 w-7" />
                                </div>
                                <span className="text-[10px] font-black uppercase text-white/20 tracking-[0.2em] mt-1">Live Feed</span>
                            </div>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                ...
                <Card className="lg:col-span-2 glass-card border-white/10 shadow-2xl rounded-[3rem] overflow-hidden">
                    <CardHeader className="p-10 border-b border-white/5 flex flex-row items-center justify-between bg-white/5">
                        <div>
                            <CardTitle className="text-3xl font-black text-white italic tracking-tighter">Clinical Queue</CardTitle>
                            <p className="text-white/40 text-sm font-bold mt-1">Prioritized patient assessments for today.</p>
                        </div>
                            ...
                            {[1, 2, 3, 4].map((i) => (
                                <div key={i} className="p-8 hover:bg-white/5 transition-all flex items-center justify-between group cursor-pointer border-l-4 border-transparent hover:border-cyan-500">
                                    <div className="flex items-center gap-6">
                                        ...
                                        <div>
                                            <h4 className="font-black text-white text-lg group-hover:text-cyan-400 transition-colors italic tracking-tight">Patient Registry #{1000 + i}</h4>
                                            <p className="text-[10px] font-black text-white/40 flex items-center gap-2 uppercase tracking-widest mt-1">
                                                <Activity className="h-3 w-3 text-cyan-500" /> Molecular Lipid Analysis
                                            </p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-12">
                                        <div className="hidden md:block text-right">
                                            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/20 mb-1">Time Assigned</p>
                                            <p className="text-sm font-black text-white tracking-widest">09:30 AM</p>
                                        </div>
                                        <Button variant="ghost" className="rounded-2xl h-14 font-black uppercase text-[10px] tracking-widest gap-3 text-cyan-500 hover:bg-cyan-500/10 px-8 border border-white/5 bg-white/5">
                                            Review Case <ArrowRight className="h-4 w-4" />
                                        </Button>
                                    </div>
                                </div>
                            ))}
                ...
                <div className="space-y-8">
                    <Card className="glass-card border-white/10 shadow-2xl rounded-[3rem] overflow-hidden">
                        <CardHeader className="p-8 pb-4">
                            <CardTitle className="text-xl font-black text-white flex items-center gap-3 italic">
                        ...
                        <CardContent className="p-8 pt-0 space-y-6">
                            {notifLoading ? (
                                ...
                                <div className="text-center py-12">
                                    ...
                                    <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em]">Clinical feed is empty</p>
                                </div>
                            )}
                        </CardContent>
                    </Card>

                    <Card className="bg-gradient-to-br from-cyan-600 to-emerald-700 shadow-2xl rounded-[3rem] p-10 text-white relative overflow-hidden group border-white/10">
                        <div className="relative z-10">
                            <Stethoscope className="h-12 w-12 text-white/30 mb-6 group-hover:scale-110 transition-transform duration-500" />
                            <h3 className="text-3xl font-black mb-4 leading-tight italic tracking-tighter">Diagnostic<br />Protocols</h3>
                            <p className="text-white/70 text-sm font-bold mb-8 leading-relaxed">Access the latest molecular screening updates across the network.</p>
                            <Button className="w-full bg-white text-cyan-700 font-black uppercase tracking-widest text-[10px] h-14 rounded-2xl shadow-2xl hover:bg-slate-50 hover:-translate-y-1 transition-all">
                                Open Clinical Desk
                            </Button>
                        </div>
